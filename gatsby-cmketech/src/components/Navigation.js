...
const Navigation = ({ toggleNav }) => {
  ...
  const SubMenu = ({ data }) => {
    return (
      <ul role="menubar" aria-hidden="true" className={`nav__list nav__list--secondary ${subNavShow == data.id ? "showSubNav" : ""}`}>
    ...
    <nav id="nav" className={`header__nav nav__wrapper ${toggleNav ? "nav-open" : " "}`} aria-label="site navigation" role="navigation">
      <ul role="menubar" aria-hidden="false" className="nav nav__list nav__list--primary">
        {wpMenu.menuItems.nodes.map((mainItem) =>
          !mainItem.parentId ? (
            <li key={mainItem.id} role="menuitem" className={`nav__item nav__item--primary ${mainItem.childItems.nodes.length !== 0 ? "hasChild" : ""}`}>
              ...
              {mainItem.childItems.nodes.length !== 0 ? <SubMenu data={mainItem} menuIndex={subNavShow} /> : null}
            </li>
          ) : null
        )}
      </ul>
